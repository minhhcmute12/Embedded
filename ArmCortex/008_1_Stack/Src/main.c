 ...
 * Có nghĩa là trong chế độ Handler mode thì SPSEL = 0 là luôn luôn.
 *
...
/**
 * Stack Exercise(V43+V44) -Ví dụ hoạt động của stack
 * ----------------------------------------------------
 * Khi chạy thì chọn phần Debug -> Showvỉew -> Register ->quan sát các thay đổi value trong các thanh ghi
 */
#include<stdio.h>

//Định nghĩa phân vùng stack
#define SRAM_START 				0x20000000U
#define SRAM_SIZE  				(128 * 1024)
#define SRAM_END  				( (SRAM_START) + (SRAM_SIZE) )
#define STACK_START SRAM_END

//Tao 1 phân cùng stack trong đó 1 nửa dùng cho MSP và 1 nửa dùng cho PSP
...
/**
 * Hàm nội tuyến yêu cầu M Cortex chuyển từ sử dụng MSP(SP) sang PSP
 * naked: hàm nội tuyến dạng rỗng, hay dùng để thay đổi dạng stack pointer
 */
__attribute__((naked)) void change_sp_to_psp(void)
{
	//Phân vùng stack theo ý muốn bằng hàm nội tuyến
	__asm volatile(".equ SRAM_END, (0x20000000 + ( 128 * 1024))");
	__asm volatile(".equ PSP_START , (SRAM_END-512)");
	__asm volatile("LDR R0,=PSP_START");   //tạo value PSP_START cho thanh ghi R0, LDR: từ khóa lưu value dùng cho 32bit

	__asm volatile("MSR PSP, R0");		//đọc và lưu value thanh ghi R0 vào PSP, MSR: từ khóa lưu value từ R này -> R khác
	__asm volatile("MOV R0,#0X02");		//Tạo value 0000_0010 vào R0, MOV: từ khóa lưu value dùng cho 16bit
	//Ý nghĩa: 0000_0010 là ta sẽ set bit SPSEL lên 1 để sử dụng PSP(theo datasheet thì bit SPSEL nằm ở bit thứ 1)

	__asm volatile("MSR CONTROL,R0"); //Lưu value R0 vào thanh ghi CONTROL

	//Yêu cầu xử lý quay về nhánh xử lý chính
	__asm volatile("BX LR"); //BX: thực hiện di chuyển nhánh, LR: thanh ghi chứa địa chỉ trả về thực hiện hàm chính(main)
	//Các value đc thực thi ở hàm nội tuyến thì sau khi có lệnh này sẽ đc lưu vào PC và tiếp tục thực hiện hàm chính.

}

/**
 * Hàm ngoại lệ: phòng việc lỗi xảy ra khi có lỗi trong thực thi hàm nội tuyến
 */
void generate_exception(void)
{
	/* execute SVC instruction to cause SVC exception */
	__asm volatile("SVC #0X2"); //SVC: từ khóa kích hoạt ngoại lệ
}
...
/* from here onwards PSP will be used for stack activities */
	...
	ret = fun_add(1, 4, 5, 6);
	//Chương trình khi chạy đến đây sẽ báo lỗi vì khi ở hàm nội tuyến bạn đã thay đổi sang sử dụng PSP
	//nhưng chương trình đang ở dạng Handler mode nên bắt buốc stack pointer luôn ở dạng MSP
	//Hàm generate_exception sẽ đc kích hoạt lúc này
...
/**
 * Hàm đc kích hoạt khi hàm nội tuyến generate_exception đc kích hoạt
 * Dùng để thông báo cho User đang có lỗi khi xử lý nội tuyến
 */
