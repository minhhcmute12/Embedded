...
//Global array of records -> Mảng cấu trúc
...
int delete_record(STUDENT_INFO_t *record, int8_t max_record)
{
     ...
     for(int i = 0 ; i < max_record ; i++)
    {
        ...
        {
            delete_flag = 1;
            memset(&record[i],0,sizeof(STUDENT_INFO_t)); //đưa value record muốn xóa về 0
            break;
        }
    }
...
/**
 * Trong ngôn ngữ lập trình C, hàm memset() được sử dụng để thiết lập các giá trị của một vùng nhớ
 * liên tiếp thành một giá trị cụ thể.
 * Cú pháp của hàm memset() như sau:
 * void *memset(void *ptr, int value, size_t num);
 * Trong đó:
 * ptr: là con trỏ đến vùng nhớ cần thiết lập giá trị.
 * value: là giá trị cần thiết lập.
 * num: là số byte cần thiết lập giá trị.
 *
 * Hàm memset() sẽ trả về con trỏ đến vùng nhớ được thiết lập giá trị.
 * Ví dụ, để thiết lập một mảng gồm 10 số nguyên thành giá trị 0, ta có thể sử dụng hàm memset() như sau:
 * int arr[10]; memset(arr, 0, sizeof(arr));
 *
 * Lưu ý rằng hàm memset() chỉ làm việc với các giá trị byte. Do đó, nếu bạn muốn thiết lập các giá trị
 * của một vùng nhớ kiểu dữ liệu khác, bạn có thể sử dụng ép kiểu (typecasting) để chuyển đổi địa chỉ
 * của vùng nhớ sang kiểu dữ liệu void*, rồi sử dụng hàm memset() để thiết lập các giá trị byte của
 * vùng nhớ đó. Tuy nhiên, khi làm việc với kiểu dữ liệu phức tạp, như các struct hay class, bạn nên
 * sử dụng các phương thức hoặc hàm thiết lập giá trị của từng thành phần của kiểu dữ liệu đó một
 * cách riêng biệt để đảm bảo tính chính xác và an toàn.
 */
