...
#define __weak 	__attribute__((weak))		//Định nghĩa hàm hoặc biến có thể đc định nghĩa từ file liên kết khác
 ...
 * Lưu ý: Nên định nghĩa cấu trúc thanh ghi trước khi cấu hình xung clock
 */
/*
...
/*
 * 23_1 Peripheral Register definition structure for I2C
 * RM407 -> 27.6.11 I2C register map
 */
typedef struct
{
...
/**
 * 18_1 IRQ(Interrupt request) Number of SRTM32F407x MCU for SPIx
 * Tiến hành tra datasheet RM407->12.2->Table61 để xác định Position mà SPIx sẽ kết nối tới NVIC_IRQx
 */
 ...
 * APB2 Bus là thanh ghi đc quản lý các SPI1, APB1 Bus là thanh ghi đc quản lý các SPIx(2,3)
 * Cần xem datasheet thanh ghi RCC_APB1ENR và RCC_APB2ENR để bit vị trí các SPIx để disable cho chính xác
 */
#define SPI1_PCLK_DI()		(RCC->APB2ENR &= ~(1 << 12))		//cho bit12(SPI1) của thanh ghi APB2ENR lên 0 (disable clock)
#define SPI2_PCLK_DI()		(RCC->APB1ENR &= ~(1 << 14))		//cho bit14(SPI1) của thanh ghi APB1ENR lên 0 (disable clock)
#define SPI3_PCLK_DI()		(RCC->APB1ENR &= ~(1 << 15))

/*****************************************************************************************
 * 23_1 Bit Position definition of I2C Peripheral(V182)
...
/**
 * 23_2_Macros function to reset clock SPI Peripheral
 * APB1(I2C1,I2C2,I2C3->RCC_APB1RSTR) Bus là thanh ghi đc treo(quản lý) các I2Cx
 */
#define I2C1_REG_RESET()	do { (RCC->APB1RSTR |= (1 << 21)) ; (RCC->APB1RSTR &= ~(1 << 21)); } while(0)		//RCC_APB1RSTR->bit21
#define I2C2_REG_RESET()	do { (RCC->APB1RSTR |= (1 << 22)) ; (RCC->APB1RSTR &= ~(1 << 22)); } while(0)		//RCC_APB1RSTR->bit22
#define I2C3_REG_RESET()	do { (RCC->APB1RSTR |= (1 << 23)) ; (RCC->APB1RSTR &= ~(1 << 23)); } while(0)		//RCC_APB1RSTR->bit23

/**
 * 23_3 Clock Disable Macros for SPIx peripherals
 * APB2 Bus là thanh ghi đc quản lý các SPI1, APB1 Bus là thanh ghi đc quản lý các SPIx(2,3)
 * Cần xem datasheet thanh ghi RCC_APB1ENR và RCC_APB2ENR để bit vị trí các SPIx để disable cho chính xác
 */
#define I2C1_PCLK_DI()		(RCC->APB1ENR &= ~(1 << 21))		//cho bit21(I2C1) của thanh ghi APB1ENR lên 0 (Disable clock)
