...
typedef struct
{
	...
	//Support for Interrupt -> Lưu data cần truyền hoặc nhận như một biến toàn cục
...
/*
 * SPI application states (Định nghĩa trạng thái SPI đang đc sử dụng)
 */
#define SPI_READY 					0
...
/*
 * Define Possible SPI Application events
 */
...
/*
 * State of Data Send and Receive for SPI Interrupt
 */
...
//SPI for Interrupt
void SPI_CloseTransmisson(SPI_Handle_t *pSPIHandle);						//Đóng việc interrup truyền
void SPI_CloseReception(SPI_Handle_t *pSPIHandle);							//Đóng việc interrup nhận
void SPI_ClearOVRFlag(SPI_RegDef_t *pSPIx);									//Xóa cờ bit OVR-> cho phép TX-RX tiếp theo
