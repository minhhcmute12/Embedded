...
typedef struct
{
	...
	GPIO_PinConfig_t GPIO_PinConfig;	//biến này giữ cài đặt cấu hình chân GPIO

}GPIOx_Handle_t;

/**
 * 4_GPIO Register Macros - Tiến hành định nghĩa(Macros) trạng thái bit cho các register liên quan đến GPIO
...
//Các mode khác có thể định nghĩa
 ...
 */
void GPIO_IRQConfig(uint8_t IRQNumber, uint8_t IRQPriority, uint8_t EnorDi);			//APIs Interrupt config(cấu hình) - Yêu cầu ngắt
void GPIO_IRQHangling(void);				//APIs Interrupt Hangling(xử lý) - thực hiện công việc khi ngắt xảy ra
